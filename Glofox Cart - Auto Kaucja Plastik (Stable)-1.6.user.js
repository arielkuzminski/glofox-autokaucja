...
// @namespace    glofox
// @version      1.6
// @description  Po dodaniu produktu z kaucją (np. woda) otwiera modal i dodaje Kaucja plastik. Synchronizuje ilość kaucji z ilością produktów z [k].
...
// @updateURL    https://raw.githubusercontent.com/arielkuzminski/glofox-autokaucja/master/Glofox%20Cart%20-%20Auto%20Kaucja%20Plastik%20(Stable)-1.6.user.js
// @downloadURL  https://raw.githubusercontent.com/arielkuzminski/glofox-autokaucja/master/Glofox%20Cart%20-%20Auto%20Kaucja%20Plastik%20(Stable)-1.6.user.js
// ==/UserScript==
...
(function () {
  ...
  function getCartLineItems() {
    return [...document.querySelectorAll('[data-testid^="cart_line_item_"]')];
  }

  /** @param {Element} lineItem @returns {number} */
  function getCartItemQty(lineItem) {
    const selItem = lineItem.querySelector(".ant-select-selection-item");
    return parseInt(
      selItem?.getAttribute("title") || selItem?.textContent || "1",
      10,
    );
  }
  ...
  function isDepositInCart() {
    return getDepositLineItem() !== null;
  }

  /**
   * Znajduje line-item kaucji w koszyku.
   * @returns {Element|null}
   */
  function getDepositLineItem() {
    return (
      getCartLineItems().find((li) => {
        const name = (li.querySelector("h4")?.innerText || "").trim();
        return /kaucja/i.test(name);
      }) || null
    );
  }

  /**
   * Sumuje ilości produktów z [k] w koszyku (pomijając kaucję).
   * @returns {number}
   */
  function countRequiredDeposits() {
    let total = 0;
    for (const li of getCartLineItems()) {
      const name = (li.querySelector("h4")?.innerText || "").trim();
      if (/kaucja/i.test(name)) continue;
      if (!DEPOSIT_MARKER_REGEX.test(name)) continue;
      total += getCartItemQty(li);
    }
    return total;
  }

  /**
   * Ustawia ilość kaucji przez Ant Design Select dropdown.
   * Blokada isSettingQuantity zapobiega równoległym wywołaniom, które powodują
   * dwukrotne kliknięcie selectora (open → natychmiastowe close) i brak dropdownu.
   * @param {Element} lineItem
   * @param {number} qty
   * @returns {Promise<boolean>}
   */
  async function setDepositQuantity(lineItem, qty) {
    if (isSettingQuantity) return false;
    isSettingQuantity = true;

    try {
      const selector = lineItem.querySelector(".ant-select-selector");
      if (!selector) return false;

      // Ant Design otwiera dropdown na mousedown (nie na click).
      // element.click() NIE dispatchuje mousedown, stąd dropdown się nie otwierał.
      ...
      // Czekamy na dropdown (polling co 100ms, max 1s) — 250ms było za krótkie
      let dropdown = null;
      for (let i = 0; i < 10; i++) {
        await sleep(100);
      ...
      if (!dropdown) {
        console.warn("[GLOFOX] Dropdown ilości nie pojawił się.");
        return false;
      }
      ...
      const target = [...options].find(
        (opt) =>
          opt.getAttribute("title") === String(qty) ||
          opt.textContent.trim() === String(qty),
      );

      if (target) {
        target.click();
        console.log("[GLOFOX] Ustawiono ilość kaucji na:", qty);
        return true;
      }

      // Zamknij przez Escape — nie klika selectora ponownie (unika toggle i mutacji)
      console.warn("[GLOFOX] Nie znaleziono opcji ilości:", qty);
  ...
  /**
   * Otwiera modal i dodaje kaucję do koszyka (1 sztukę).
  ...
  // ── Synchronizacja ilości kaucji ─────────────────────────────────

  /**
   * Porównuje wymaganą ilość kaucji z aktualną i wyrównuje.
   * Jeśli kaucji nie ma w koszyku — dodaje ją (addDeposit) i ustawia qty.
   * Jeśli jest, ale ze złą ilością — zmienia qty przez dropdown.
   * @returns {Promise<void>}
   */
  async function syncDepositQuantity() {
    const required = countRequiredDeposits();
    const depositItem = getDepositLineItem();
    const current = depositItem ? getCartItemQty(depositItem) : 0;

    console.log(
      "[GLOFOX] Sync kaucji: wymagane =",
      required,
      ", aktualne =",
      current,
    );
    ...
    // Brak produktów z [k] — nie usuwamy kaucji (mogła być dodana ręcznie)
    if (required === 0) return;

    if (!depositItem && required > 0) {
      // Kaucji nie ma w koszyku → dodaj, potem ustaw qty
      await addDeposit();
      if (required > 1) {
        await sleep(500);
        const newItem = getDepositLineItem();
        if (newItem) await setDepositQuantity(newItem, required);
      }
      return;
    }

    if (depositItem && current !== required) {
      // Kaucja jest, ale zła ilość → zmień qty przez dropdown
      await setDepositQuantity(depositItem, required);
    }
  }

  /** Cart watcher: obserwuje zmiany w koszyku i synchronizuje ilość kaucji. */
  const cartSyncObserver = new MutationObserver(() => {
    if (!isCartPage()) return;
    if (isAddingDeposit) return;
    if (isSettingQuantity) return;

    clearTimeout(syncDebounceTimer);
    syncDebounceTimer = setTimeout(() => {
      if (!isAddingDeposit && !isSettingQuantity) syncDepositQuantity();
    }, 800);
  });
  cartSyncObserver.observe(document.body, { childList: true, subtree: true });
  ...
  document.addEventListener(
    ...
    async (e) => {
      ...
      if (hasDepositMarker(resultEl)) {
        ...
        if (!added) {
          console.warn(
            "[GLOFOX] Produkt nie pojawil sie w koszyku na czas, probuje zsynchronizowac kaucje mimo to.",
          );
        }

        await sleep(300);
        await syncDepositQuantity();
      }
    },
    true,
  );

  console.log("[GLOFOX] Auto-Kaucja Plastik (Stable) v1.6 aktywna ✅");
})();
